...
export class StoryReportGenerator {
    ...
    async generatePDF(options: Partial<ExportOptions> = {}): Promise<Blob> {
        ...
        if (this.config.author) {
            ...
            doc.text(`By ${this.config.author}`, pageWidth / 2, 75, {
                align: 'center',
            });
        }
        ...
        for (let i = 0; i < this.keyframes.length; i++) {
            ...
            if (exportOptions.includeNarrative) {
                const textY = exportOptions.includeScreenshots
                    ? margin + 100
                    : margin;
                    ...
                    >
                        ...
                        <div className="keyframe-info">
                            ...
                            <div className="keyframe-meta">
                                ...
                                {keyframe.pitch && (
                                    <span>Pitch: {keyframe.pitch}°</span>
                                )}
                                {keyframe.bearing && (
                                    <span>Bearing: {keyframe.bearing}°</span>
                                )}
                            </div>
                        </div>
                        <button
                ...
                <div className="keyframe-editor">
                    ...
                    <label>
                        ...
                        <input
                            type="text"
                            value={
                                keyframes.find((k) => k.id === selectedKeyframe)
                                    ?.title ?? ''
                            }
                            onChange={(e) =>
...
export const StoryModeViewer: React.FC<{
    ...
    return (
        <div
            className={`story-mode-viewer ${isFullscreen ? 'fullscreen' : ''}`}
        >
