...
export class ContextAwareLayerManager {
  ...
  private updateLayer(id: string): void {
    ...
    const renderMode = getOptimalRenderMode(this.currentZoom, this.currentDensity);
  ...
  getCurrentContext(): {
    ...
    return {
      ...
      renderMode: getOptimalRenderMode(this.currentZoom, this.currentDensity),
    };
  }
}
 ...
 */
export function useContextAwareStyling(
  featureCount: number
): {
  zoom: number;
  config: ReturnType<typeof getZoomConfig>;
