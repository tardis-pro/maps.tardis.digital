...
export class HybridLayerManager {
   ...
   */
  getSourceConfig(sourceId: number, strategy: RenderStrategy): SourceSpecification {
    if (strategy === RenderStrategy.RASTER) {
      // Raster source (COG)
      return {
        type: 'raster',
        tiles: [`${this.config.rasterTilerUrl}/tiles/{z}/{x}/{y}?url=postgresql://...`],
        tileSize: 256,
        scheme: 'xyz',
      };
    } else {
      ...
      return {
        type: 'vector',
        tiles: [`${this.config.vectorTilerUrl}/tile?source=${sourceId}&z={z}&x={x}&y={y}`],
        minzoom: 0,
        maxzoom: 22,
      };
    }
  ...
  getTileUrl(
    ...
    const actualStrategy = strategy ?? info?.recommendedStrategy ?? RenderStrategy.VECTOR;
...
export function createLayerManager(config?: Partial<TilingConfig>): HybridLayerManager {
