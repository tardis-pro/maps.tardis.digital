...
const isShapefile = (file: File) => file.name.endsWith('.shp');
const isKML = (file: File) => file.name.endsWith('.kml');
...
const DataManager: React.FC = () => {
  ...
  const [selectedTab, setSelectedTab] = useState<'file' | 'api' | 'database'>('file');
  ...
  const handleFiles = async (files: File[]) => {
    for (const file of files) {
      try {
        ...
        if (isGeoJSON(file)) {
          data = await readGeoJSON(file, (progress) => {
          ...
          data = await readCSV(file, (progress) => {
            setUploadProgress(prev => ({ ...prev, [file.name]: progress }));
          });
          ...
          if (data.length > 0) {
            Object.keys(data[0]).forEach(key => {
              properties[key] = { type: typeof data[0][key] };
            });
          }
  ...
  // Read GeoJSON file
  const readGeoJSON = (file: File, onProgress: (progress: number) => void): Promise<any> => {
  ...
  // Read CSV file
  const readCSV = (file: File, onProgress: (progress: number) => void): Promise<any[]> => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      ...
      <div className="data-manager-content">
        ...
        {selectedTab === 'file' && (
          <div className="upload-section">
            ...
            >
              ...
              <span className="supported-formats">Supported formats: GeoJSON, CSV, TIF, JPG, PNG</span>
              <input 
                type="file" 
                ref={fileInputRef} 
          ...
          <div className="database-section">
            ...
            <div className="form-group">
              ...
              <textarea 
                ...
                onChange={(e) => setDatabaseConfig({ ...databaseConfig, query: e.target.value })}
              />
            </div>
            <button 
              className="connect-btn"
              onClick={handleDatabaseConnect}
              disabled={!databaseConfig.host || !databaseConfig.database}
            >
              Connect to Database
            </button>
          </div>
        ...
        <div className="datasets-section">
          ...
          <div className="datasets-list">
              ...
              <div key={dataset.id} className="dataset-card">
                ...
                <div className="dataset-info">
                  <div className="dataset-properties">
                    <span>Properties: {Object.keys(dataset.properties).length}</span>
                    {dataset.type === 'geojson' && dataset.data?.features && (
                      <span>Features: {dataset.data.features.length}</span>
                    )}
                    {dataset.type === 'csv' && dataset.data && (
                      <span>Rows: {dataset.data.length}</span>
                    )}
                  </div>
                  <div className="dataset-source">
                    {dataset.sourceUrl ? (
                      <span title={dataset.sourceUrl}>API Source</span>
                    ) : (
            ...
            {datasets.length === 0 && (
              <p className="empty-state">No datasets imported yet. Use the tools above to add data.</p>
            )}
          </div>
        </div>
      </div>
