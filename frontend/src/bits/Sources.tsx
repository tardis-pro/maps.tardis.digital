...
const CatalogDial: React.FC = () => {
    ...
    const fetchCatalog = async () => {
      ...
      try {
        ...
        if (data.tiles) {
          ...
          const items: CatalogItem[] = Object.keys(data.tiles).map(key => ({
            id: key,
  ...
  const categories = Array.from(new Set(catalogItems.map(item => item.type)));
        ...
        {filteredItems.length > 0 ? (
          filteredItems.map((item) => (
            ...
            >
              ...
              <div className="text-xs text-gray-400 mt-1">Type: {item.type}</div>
            </motion.div>
          ))
        ) : (
