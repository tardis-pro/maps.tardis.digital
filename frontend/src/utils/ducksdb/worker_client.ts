...
import type { DuckDBConfig, DuckDBBundle, ColumnType, RowDataType } from '@duckdb/duckdb-wasm';
...
export class DuckDBWorkerClient {
  ...
  constructor(config?: WorkerClientConfig) {
    ...
    this.worker.onerror = (error) => {
      this.rejectReady(error instanceof Error ? error : new Error('Worker error'));
    };
 ...
 */
export async function getDuckDBClient(config?: WorkerClientConfig): Promise<DuckDBWorkerClient> {
  if (!globalClient) {
 ...
 */
export async function loadParquet(name: string, data: Uint8Array): Promise<void> {
  const client = await getDuckDBClient();
  await client.registerFile(name, data);
  await client.query(`CREATE TABLE ${name.replace(/[^a-zA-Z0-9]/g, '_')} AS SELECT * FROM read_parquet('${name}')`);
}

/**
 * Load a CSV file into DuckDB
 */
export async function loadCSV(name: string, data: Uint8Array, options?: { delimiter?: string; header?: boolean }): Promise<void> {
  const client = await getDuckDBClient();
  await client.registerFile(name, data);
  const delimiter = options?.delimiter ?? ',';
  const header = options?.header ?? true;
  const tableName = name.replace(/[^a-zA-Z0-9]/g, '_');
  await client.query(`CREATE TABLE ${tableName} AS SELECT * FROM read_csv_auto('${name}', delimiter='${delimiter}', header=${header})`);
}
