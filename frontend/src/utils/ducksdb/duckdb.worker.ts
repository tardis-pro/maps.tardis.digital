...
import type { DuckDBConfig, AsyncDuckDB, AsyncDuckDBConnection, RowDataType } from '@duckdb/duckdb-wasm';
...
async function registerFile(name: string, data: Uint8Array): Promise<void> {
  ...
  try {
    ...
    postMessage({ type: 'result', data: [{ success: true, name }], types: [] });
  } catch (error) {
    postMessage({
      type: 'error',
      message: error instanceof Error ? error.message : 'File registration failed',
    });
  }
}
...
async function insertArrow(name: string, arrow: Uint8Array): Promise<void> {
  ...
  try {
    ...
    postMessage({
      type: 'error',
      message: error instanceof Error ? error.message : 'Arrow insert failed',
    });
  }
}
