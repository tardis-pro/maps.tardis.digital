...
const setupCtx = React.createContext<Resolvable<
    duckdb.AsyncDuckDB,
    duckdb.InstantiationProgress
> | null>(null);
const resolverCtx = React.createContext<Resolver<duckdb.AsyncDuckDB> | null>(
    null
);

export const useDuckDB = (): Resolvable<
    duckdb.AsyncDuckDB,
    duckdb.InstantiationProgress
> => React.useContext(setupCtx)!;
export const useDuckDBResolver = (): Resolver<duckdb.AsyncDuckDB> =>
...
export const DuckDBProvider: React.FC<DuckDBProps> = (props: DuckDBProps) => {
    ...
    const [setup, updateSetup] = React.useState<
        Resolvable<duckdb.AsyncDuckDB, duckdb.InstantiationProgress>
    ...
    const inFlight = React.useRef<Promise<duckdb.AsyncDuckDB | null> | null>(
        null
    );
                ...
                await next.instantiate(
                    ...
                    (p: duckdb.InstantiationProgress) => {
                        try {
                            ...
                            console.warn(
                                `progress handler failed with error: ${e.toString()}`
                            );
                        }
                    }
                );
